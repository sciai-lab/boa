...
def compute_density_optimization_metrics(
    ...
    metric_dict["signed_energy_error"] = signed_energy_error
    metric_dict["total_energy_error"] = np.abs(signed_energy_error)
    energy_differences = total_energies - ground_state_energy.total_energy
    metric_dict["min_energy_error"] = energy_differences.min()
    metric_dict["l2_norm"] = callback.l2_norm[stopping_index]
    metric_dict["minimum_l2_norm"] = min(callback.l2_norm)
    metric_dict["stopping_index"] = stopping_index
    metric_dict["gradient_norm"] = callback.gradient_norm[stopping_index]
...
def run_density_optimization(
...
) -> tuple[dict, ConvergenceCallback, Energies]:
    ...
    if hasattr(sample, "grid_level"):
        grid = dft.Grids(mol)
        grid.level = sample.grid_level
        grid.prune = sample.grid_prune
        grid.build()
    else:
        grid = None
    ...
    energy_functional = func_factory.construct(
        mol, sample.coulomb_matrix, sample.nuclear_attraction_vector, grid, ao
    )
    # Optimization
    final_energies = optimizer.optimize(sample, energy_functional, callback)
    ...
    logger.info(
        ...
        f"ΔE={metric_dict['signed_energy_error'] * 1000:6.2g} mHa, "
        f"ρNorm={metric_dict['l2_norm']:6.2g}, "
        f"gNorm={metric_dict['gradient_norm']:7.2e}"
    )
    return metric_dict, callback, energies_label
