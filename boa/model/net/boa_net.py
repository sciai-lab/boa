...
class EdgeEmbedding(nn.Module):
    ...
    def __init__(self, basis_info: BasisInfo, channels: int = 32):
        ...
        edge_to_embedding_index = torch.arange(
            len(basis_info.atomic_numbers)**2, dtype=torch.long,
        ).view(len(basis_info.atomic_numbers), len(basis_info.atomic_numbers))
        self.register_buffer(
            "edge_to_embedding_index",
            edge_to_embedding_index
        )
    ...
    def forward(self, batch) -> Tensor:
        ...
        edge_a = torch.zeros(edge_index.shape[1], self.basis_dim_per_atom.max().item(), self.channels, device=edge_index.device, dtype=self.embedding.weight.dtype)
        edge_b = torch.zeros(edge_index.shape[1], self.basis_dim_per_atom.max().item(), self.channels, device=edge_index.device, dtype=self.embedding.weight.dtype)
        ...
        for i, a in enumerate(self.basis_info.atomic_numbers):
            for j, b in enumerate(self.basis_info.atomic_numbers):
                ...
                edge_a[edge_index_mask, :self.scalar_dims[i]] = scalar_features_a[edge_index_mask, :self.scalar_dims[i]]
                edge_b[edge_index_mask, :self.scalar_dims[j]] = scalar_features_b[edge_index_mask, :self.scalar_dims[j]]
...
class EdgeEmbeddingV2(nn.Module):
    ...
    def forward(self, batch) -> Tensor:
        ...
        edge_features_a, edge_features_b = edge_features[..., :self.channels], edge_features[..., self.channels:]
        node_cor_a_per_edge, node_cor_b_per_edge = node_cor_per_edge[..., :self.channels], node_cor_per_edge[..., self.channels:]

        self_loop_mask = edge_index[0] == edge_index[1]

        node_cor_a= torch.zeros(self_loop_mask.sum(), node_cor_a_per_edge.shape[1], self.channels, device=edge_index.device, dtype=edge_features_a.dtype)
        node_cor_b= torch.zeros(self_loop_mask.sum(), node_cor_b_per_edge.shape[1], self.channels, device=edge_index.device, dtype=edge_features_b.dtype)

        node_cor_a.index_add(
            0, edge_index[0], node_cor_a_per_edge
        )
        node_cor_b.index_add(
            0, edge_index[0], node_cor_b_per_edge
        )
