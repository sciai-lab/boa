...
class BasisInfo:
    """Information about an OF basis for the LCAB Ansatz (see :mod:`~mldft.ofdft.ofdft`), to be
    used in data loading. It does not directly include the actual shape of the basis functions,
    i.e. exponents, contraction coefficients etc., but the attribute :attr:`basis_dict` can be
    passed to :class:`pyscf.gto.Mole` as the ``basis`` keyword to construct a pyscf molecule.
